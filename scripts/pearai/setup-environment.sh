...
app_dir=$(pwd)
target_path="$app_dir/extensions/raspberryai-submodule/extensions/vscode"
link_path="$app_dir/extensions/raspberryai-ref"

# Run the base functionality
echo -e "\nInitializing sub-modules..."

# Check if the submodule directory already exists
if [ -d "$app_dir/extensions/raspberryai-submodule" ]; then
    echo "Removing existing raspberryai-submodule directory"
    execute "rm -rf $app_dir/extensions/raspberryai-submodule" "Failed to remove existing raspberryai-submodule directory"
fi
...
execute "cd ./extensions/raspberryai-submodule" "Failed to change directory to extensions/raspberryai-submodule"
