...
function Initialize-BaseFunctionality {
    ...
    Set-Location .\extensions\raspberryai-submodule
...
function Create-SymLink {
    Write-Host "`nCreating symbolic link 'extensions\raspberryai-ref' -> 'extensions\raspberryai-submodule\extensions\vscode'" -ForegroundColor White
...
$modulePath = Join-Path -Path $currentDir -ChildPath 'extensions\raspberryai-submodule'
$targetPath = Join-Path -Path $modulePath -ChildPath 'extensions\vscode'
$linkPath = Join-Path -Path $currentDir -ChildPath 'extensions\raspberryai-ref'
